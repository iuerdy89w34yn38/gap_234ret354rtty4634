...
class GeneralController extends Controller
{
    ...
    public function otherBankCreate(Request $request)
    {
        ...
        $banks->p_time = $request->processing_time;
 ...
 public function otherBankUpdate(Request $request, $id)
    ...
    public function HomePageUpdate(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'investor_title' => 'required|max:191',
        ...
        $data->investor_des = $request->investor_des;
        $data->latest_tran_title = $request->latest_tran_title;
        $data->latest_tran_des = $request->latest_tran_des;
    ...
    {
        $data = Investor::all();
        return view('admin.investors', compact('data'));
    }
    ...
    public function investorAdd(Request $request)
    {
        ...
        $data->pint_link = $request->pint_link;

        if($request->hasFile('image')) {
            $image = $request->file('image');
            $filename = $image->hashName();
            $location = 'assets/image/investor/' . $filename;
            Image::make($image)->fit(280, 250)->save($location);
            $data ->image = $filename;
        }
        $data->save();
    ...
    public function investorUpdate(Request $request)
        {
            ...
            $data =  Investor::findOrFail($request->id);
            ...
            if($request->hasFile('image')) {
                @unlink('assets/image/investor/' . $data->image);
                $image = $request->file('image');
                $filename = $image->hashName();
                $location = 'assets/image/investor/' . $filename;
                Image::make($image)->fit(280, 250)->save($location);
    ...
    public function investorDelete(Request $request)
    {
        ...
        @unlink('assets/image/investor/' . $investor->image);
        $investor->delete();
