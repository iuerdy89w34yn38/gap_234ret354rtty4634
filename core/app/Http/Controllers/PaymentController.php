...
class PaymentController extends Controller
{
    ...
    { 
        if($data->status==0)
        {
            ...
            $msg =  'Your Deposit of '.$data->amount.' via '.$data->gateway->name.' is Successful.';
    ...
    public function depositConfirm()
    {
        ...
        elseif ($data->gateway_id == 501) 
        {
            ...
            if($data->btc_amo==0 || $data->btc_wallet=="")
            {
                ...
                if(!$resp) 
                {
                    return redirect()->route('user.dashboard')->withErrors( 'BLOCKCHAIN API HAVING ISSUE. PLEASE TRY LATER');
                }
        ...
        elseif($data->gateway_id ==502)
		{
            ...
            {
                return back()->withErrors( 'Failed to Process');
            }
        ...
        elseif($data->gateway_id == 512) 
        {
            ...
            {
                return redirect()->route('user.dashboard')->withErrors('Unexpected Error! Please Try Again');
				exit();
			}
    ...
    public function ipnstripe(Request $request)
    {
        ...
        try 
        {
            ...
            {
                return redirect()->route('user.dashboard')->withErrors( $e->getMessage());
            }
    ...
    public function purchaseVogue($trx, $type)
    {
        ...
        if ($type == 'error') redirect()->route('home')->withErrors( 'Transaction Failed, Ref: ' . $trx);
                ...
                if($result['data']){
                    if ($result['data']['status'] == 'success') {
                        ...
                        if ($am == $sam) {
                            ...
                            return redirect()->route('home')->withErrors( 'Less Amount Paid. Please Contact With Admin');
                        }
                    }else{
                        return redirect()->route('home')->withErrors( $result['data']['gateway_response']);
                    }
                }else{
                    return redirect()->route('home')->withErrors( $result['message']);
                }

            }else{
                return redirect()->route('home')->withErrors( 'Something went wrong while executing');
            }
        }else{
            return redirect()->route('home')->withErrors( 'Something went wrong while executing');
        }
