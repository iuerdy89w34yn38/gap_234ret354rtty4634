...
@section('content')
    ...
    <section id="investors">
        <div class="container">
            ...
            <div class="row">
                        ...
                        <div class="box">
                            ...
                            <div class="info">
                                <ul class="list-group text-center">
                                    ...
                                    <li class="list-group-item">
                                        @lang('Charge'): <strong>{{$gnl->cur_symbol}} {{$gate->fixed_charge}}</strong> +
                ...
                @foreach($mgates as $mgate)
                    <div class="col-md-6 col-lg-3">
                        <div class="box">
                            <div class="image">
                                <img class="img-fluid" src="{{asset('assets/image/manual_gateway')}}/{{$mgate->image}}"
                                     style="width:100%;"/>
                            </div>
                            <div class="info">
                                <ul class="list-group text-center">
                                    ...
                                    <li class="list-group-item">
                                        @lang('Limit'): <strong>{{$gnl->cur_symbol}} {{$mgate->minamo}}</strong> ~
                                        <strong>{{$gnl->cur_symbol}} {{$mgate->maxamo}}</strong>
                                    </li>
                                    <li class="list-group-item">
                                        @lang('Charge'): <strong>{{$gnl->cur_symbol}} {{$mgate->fixed_charge}}</strong>
                                        + <strong>{{$mgate->percent_charge}} %</strong>
                                    </li>

                                </ul>
                                <div class="card-footer">
                                    <button data-toggle="modal" data-name="{{__($mgate->name)}}"
                                            data-gate="{{$mgate->id}}" data-target="#mdepoModal"
                                            class="btn submit-btn mdepoButton viweBtn" style="width:100%;">
                                        @lang('Deposit Now')
                                    </button>
                                </div>
                            </div>
                                        ...
                                        @foreach($deposit as $log)
                                            <tr>
                                                ...
                                                @else
                                                    ...
                                                    @php
                                                        $mgate = \App\GatewayManual::find($log->gateway_id)
                                                    @endphp
                ...
                <div class="modal-body">
                    <form action="{{route('user.deposit.data-insert')}}" method="POST">
                        ...
                        <div class="form-group">
                            <h5><label>@lang('Enter Deposit Amount')</label></h5>
    ...
    <div class="modal fade" id="mdepoModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
         aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="mModalLabel"></h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{route('user.deposit.data-manual')}}" method="POST">
                        @csrf
                        <input type="hidden" name="gateway" id="mgateWay"/>
                        <div class="form-group">
                            <h5><label>@lang('Enter Deposit Amount')</label></h5>
...
@section('script')
    <script>
        ...
        $(document).ready(function () {
            $(document).on('click', '.mdepoButton', function () {
                $('#mModalLabel').text($(this).data('name'));
                $('#mgateWay').val($(this).data('gate'));
            });
        });
    </script>
