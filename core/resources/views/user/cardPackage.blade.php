...
.logo {
  ...
  z-index: 20;
  background-image: url("{{url('/')}}/assets/image/logo.svg");
}
...
.paypal_center {
  ...
  z-index: 1;
  filter: drop-shadow(-6.4px 6.2px 8px rgba(0, 0, 0, 0.6));
  background-image: url("{{url('/')}}/assets/image/favicon.png");
}
...
.visa_icon {
  ...
  filter: drop-shadow(-6.4px 6.2px 8px rgba(0, 0, 0, 0.6));
  background-repeat: no-repeat;
  background-image: url("{{url('/')}}/assets/image/visa.png");
  background-size: 100%;
}
.thickness {
  width: 453px;
...
ul.checkList li {
    ...
    background: url(https://images.vexels.com/media/users/3/143372/isolated/preview/6e633a235ea0d523078e667b9f84f15b-blue-check-mark-by-vexels.png) no-repeat 2px 2px transparent;
 ...
 <div class="floating">
    ...
    <div class="card_body">
      ...
      <div class="card_no text">
        ...
        @endphp
        XXXX-{{ substr($actno ,4,4)}}-{{ substr($actno ,8,4)}}-{{ substr($actno ,12,4)}}
      </div>
      ...
      <div class="holder text" style="text-transform: uppercase;">{{Auth::user()->first_name}} {{Auth::user()->last_name}}</div>
      <?php 
          if ($data->days == 1){
      ?>
                        ...
                        <div class="priceBox normal">
                            ...
                            <div class="list">
                                <ul>
                                    <li>
                                        <p>
                                            Credit Card Charges:<strong>  ₿{{$data->fixed_charge}} BTC</strong> 
                                        </p>
                                    </li>
                                    <li>
                                        <p>
...
<ul class="checkList">
<li>Speed – Enjoy the life in the fast lane by making your purchases in just a few seconds</li>
<li>Convenience – Tap to pay at merchants, including convenience stores, supermarkets, etc.</li>
<li>Transactional Security – Latest encryption technology on the card protects your payment information at POS terminal</li>
</ul>
...
<li>Transactional Security – Latest encryption technology on the card protects your payment information at POS terminal</li>
      ...
      <?php } ?>
                        ...
                        -->
                            ...
                            <div class="depoButton button" data-name="{{__($data->name)}}" data-gate="{{$data->id}}" data-cprice="{{$data->fixed_charge}}" data-toggle="modal"  data-target="#depoModal">
                                <a href="#">
                                    @lang('Choose')
                                </a>
                            </div>
                ...
                <div class="modal-body">
                    <form action="{{route('user.card.store')}}" method="POST">
                        ...
                        <div class="form-group" >
                            <h4> <label>@lang('Please Send the Card Amount') = ₿<span id="price"></span> BTC to Wallet Address Below:</label></h4>
                             @php
                                $WalletAddress = \App\GatewayManual::find(1);
                             @endphp
                            <center>
                              <br>

                             <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={{$WalletAddress->dec}}">
                              <br>
                              <br>
                             {{$WalletAddress->dec}}
                              <br>
                            ...
                            <h4> <label>@lang('Enter Transaction ID*')</label></h4>
                            <div class="input-group-append">
                                <input type="text" name="amount" id="" class="form-control form-control-lg"/>
                            </div>
                        </div>

                        <div class="form-group">
                            <button type="submit" class="btn viweBtn" style="width:100%;">@lang('Send card request')</button>
                        </div>
                    </form>
                </div>
